const q10Text = "ຄວາມພະຍາຍາມຢູ່ໃສ, ຄວາມສຳເລັດຢູ່ຫັ້ນ.";
...
const GOOGLE_DRIVE_FOLDER_ID = "1uhm6y6tnHUqT00UOwpgZMLp0InkA-rmX"; // ID ຂອງໂຟນເດີ Google Drive

let currentTab = 0;

document.addEventListener('DOMContentLoaded', () => {
    setupTyping('q10Target', 'q10Input', q10Text);
    showTab(currentTab);
});
...
function checkAnswers() {
    const answers = {
        q1: 'b', q2: 'b', q3: 'b', q4: 'b',
        q5: 'b', q6: 'c', q7: 'b', q8: 'b', q9: 'c'
    };

    let theoryScore = 0;
    const totalTheoryQuestions = 9;
    ...
    // Typing Section (q10)
    let q10Val = form.elements['q10'].value.trim();

    // Normalize whitespace (convert newlines/tabs to space, collapse multiple spaces)
    q10Val = q10Val.replace(/\s+/g, ' ');

    const sim10 = calculateAccuracy(q10Val, q10Text);

    // Calculate Scores: Theory (9 pts) + Typing (1 pt)
    const theoryPoints = theoryScore;
    const typingPoints = sim10 * 1;
    ...
    renderText(document.getElementById('q10Target'), q10Text, q10Val);
...
function generateAndUploadPDF(studentName) {
    ...
    const q10Src = document.querySelector('[name="q10"]');
    if (q10Src) clone.querySelector('[name="q10"]').value = q10Src.value;
    ...
    if (q10Src) {
        const el = clone.querySelector('[name="q10"]');
        el.style.backgroundColor = calculateAccuracy(el.value.trim().replace(/\s+/g, ' '), q10Text) > 0.8 ? '#d4edda' : '#f8d7da';
    }
        ...
        .then(data => { 
            if (data.includes("Success")) {
                ...
                setTimeout(function() {
                    ...
                    renderText(document.getElementById('q10Target'), q10Text, "");
                }, 5000);
            } else {
